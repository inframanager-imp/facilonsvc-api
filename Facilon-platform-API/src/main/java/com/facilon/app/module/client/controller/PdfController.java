...
public class PdfController {
    ...
    /**
     * Generate comprehensive Account Opening Kit PDF for current user
     * Uses account-opening-kit.ftl template
     * Aligned with Laravel: /user-pdf-view/{id} (comprehensive version)
     */
    @GetMapping("/account-opening-kit")
    @Operation(summary = "Generate comprehensive Account Opening Kit PDF for current user")
    public ResponseEntity<byte[]> generateAccountOpeningKit(Authentication authentication) {
        try {
            UserPrincipal principal = (UserPrincipal) authentication.getPrincipal();
            Long userId = principal.getId();
            log.info("Generating Account Opening Kit PDF for user: {}", userId);

            byte[] pdfBytes = pdfGenerationService.generateAccountOpeningKitPdfByUserId(userId);
            
            if (pdfBytes == null || pdfBytes.length == 0) {
                log.error("Generated Account Opening Kit PDF is empty for user: {}", userId);
                return ResponseEntity.internalServerError().build();
            }
            
            log.info("Account Opening Kit PDF generated successfully. Size: {} bytes", pdfBytes.length);

            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_PDF);
            headers.setContentLength(pdfBytes.length);
            headers.setContentDispositionFormData("inline", "Account_Opening_Kit.pdf");
            headers.setCacheControl("must-revalidate, post-check=0, pre-check=0");
            headers.set("Content-Transfer-Encoding", "binary");
            headers.set("Accept-Ranges", "bytes");
            ...
            log.error("Error generating Account Opening Kit PDF for authenticated user", e);
            return ResponseEntity.internalServerError().build();
        }
    }

    /**
     * Generate Account Opening Kit print preview HTML for current user
     * Aligned with Laravel: /user-information-print-preview/{id} (comprehensive version)
     */
    @GetMapping("/account-opening-kit/preview")
    @Operation(summary = "Generate Account Opening Kit print preview for current user")
    public ResponseEntity<String> generateAccountOpeningKitPreview(Authentication authentication) {
        UserPrincipal principal = (UserPrincipal) authentication.getPrincipal();
        Long userId = principal.getId();
        log.info("Generating Account Opening Kit preview for user: {}", userId);

        String html = pdfGenerationService.generateAccountOpeningKitHtmlByUserId(userId);
    ...
    /**
     * Generate Account Opening Kit PDF by ID (admin use)
     */
    @GetMapping("/account-opening-kit/{investorId}")
    @Operation(summary = "Generate comprehensive Account Opening Kit PDF by ID")
    public ResponseEntity<byte[]> generateAccountOpeningKitById(@PathVariable Long investorId) {
        log.info("Generating Account Opening Kit PDF for investor ID: {}", investorId);

        byte[] pdfBytes = pdfGenerationService.generateAccountOpeningKitPdf(investorId);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_PDF);
        headers.setContentDispositionFormData("attachment", "Account_Opening_Kit_" + investorId + ".pdf");
        headers.setCacheControl("must-revalidate, post-check=0, pre-check=0");
